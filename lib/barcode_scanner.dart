...
class BarcodeScanner extends StatefulWidget {
  ...
  BarcodeScanner({Key key, this.title,
    this.validRectangle = const Rectangle(width: 320, height: 320),
...
class _MyHomePageState extends State<BarcodeScanner> with TickerProviderStateMixin {
  int _counter = 0;
  ...
  void _switchAnimationState(AnimationState newState, String barcodeResult) {
          ...
          if (status == AnimationStatus.completed) {
            Future<void>.delayed(const Duration(milliseconds: 300), () {
              if (_currentState != AnimationState.endSearch) {
                ...
                setState(() {});
                _showBottomSheet(barcodeResult);
              }
            });
          }
        });
  ...
  void _handleResult({
    ...
    final double imageHeight = defaultTargetPlatform == TargetPlatform.android
        ? imageSize.height
        : imageSize.width;
    ...
    for (Barcode barcode in barcodes) {
      ...
      final bool doesContain = (intersection.top % barcode.boundingBox.top < 20 && intersection.bottom % barcode.boundingBox.bottom < 20 && intersection.left % barcode.boundingBox.left < 20 && intersection.right % barcode.boundingBox.right < 20);
      ...
      if (doesContain) {
        ...
        if (_currentState != AnimationState.barcodeFound) {
          ...
          _switchAnimationState(AnimationState.barcodeFound, barcode.rawValue);
          setState(() {});
        }
        return;
  ...
  void _showBottomSheet(String resultText) {
